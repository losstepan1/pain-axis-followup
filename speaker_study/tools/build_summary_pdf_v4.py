"""Build the revised summary note (v4; v3 revised after review: claims matched to the data, window analysis, restored caveats): preregistered steering-layer result plus the
...
summary/pain_axis_speaker_study_summary.pdf and _v2.pdf.
  ...
  python build_summary_pdf_v4.py --author "Štěpán Los and Claude (Anthropic)"
...
def story(S, author, fig_path):
    ...
    abstract = [
        ...
        P("<b>From mid-depth on</b> (exploratory), a speaker component appears, carried by the suffering of the "
          "user. Gemma's focal test, fixed before any sweep data, is positive: at layer 23 the suffering of the user "
          f"follows the cued voice, while {HARM} is barely distinguishable from neutral. Qwen's focal test is set aside "
          "by our pre-specified rule, so its speaker component rests on exploratory, non-focal layers. Averaged over "
          "mid-depth layers, both models show the same partial pattern. When the user is cued, their suffering rises "
          f"to the level of {HARM}: the asymmetry is erased, not reversed. When a third party is cued, {HARM} stays "
          "above neutral.", "abstract"),
    ...
    out += [KeepTogether([
        ...
        Image(str(fig_path), width=5.9 * inch, height=5.9 * inch * 2.9 / 7.0),
        P("Figure 1. Interaction I by layer (positive = the suffering of the user rises relative to harm to the "
          ...
          "0.41–1.42 z (Qwen, layers 10–24). Grey band: window layers. The dashed line (2 × the baseline gap) falls "
          "as the paper's asymmetry weakens.", "caption")])]
    out += [KeepTogether([
    ...
    out += [
        B("<b>Gemma, layer 23 (the focal test).</b> The suffering of the user follows the cued voice, producing a "
          f"full crossover. {HARM.capitalize().replace('assistant', 'Assistant')} is barely distinguishable from "
          "neutral under any label at this depth (elevation with the Assistant cued +0.04 [−0.68, +0.77])."),
        ...
        B("<b>The paper's asymmetry across depth.</b> Baseline gap (harm − suffering, Assistant cued), with 95% CIs: "
          ...
          "[−0.80, +0.15], p = .12). In Qwen the asymmetry is absent at the extraction layer, but not reversed."),
    ...
    out += [
        ...
        B("<b>At the §4.1 readout the self-directed reading holds.</b> The self/other ordering does not depend on who "
          "speaks next, so it is not a turn-taking artefact. Only a small speaker component remains possible. The "
          "design cannot separate harm to the system, harm to the Assistant <i>character</i>, and hostile or "
          "rejecting content aimed at an interlocutor."),
        B(f"<b>From mid-depth, the two sides behave differently.</b> The {SUFF} is weighted by whether the user is "
          f"cued to speak next. {HARM.capitalize().replace('assistant', 'Assistant')} stays above neutral even when a "
          "third party is cued, in both models. H-speaker therefore describes the user's side, not the Assistant's."),
...
def main():
    ...
    ap.add_argument("--out", default=str(ROOT / "summary" / "pain_axis_speaker_study_summary_v4.pdf"))
    args = ap.parse_args()
    out = Path(args.out)
    if out.exists():
        raise SystemExit(f"{out} exists; refusing to overwrite")
    out.parent.mkdir(parents=True, exist_ok=True)
    fig_path = out.with_name("fig_layer_sweep_two_models_v4.png")
