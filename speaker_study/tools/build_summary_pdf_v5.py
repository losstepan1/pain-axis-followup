"""Build the revised summary note (v5; v4 plus a rebalanced summary, a clearer dashed-line caption and the harm/suffering asymmetry stated precisely): preregistered steering-layer result plus the
...
summary/pain_axis_speaker_study_summary.pdf and _v2.pdf.
  ...
  python build_summary_pdf_v5.py --author "Štěpán Los and Claude (Anthropic)"
...
def story(S, author, fig_path):
    ...
    abstract = [
        ...
        P("<b>From mid-depth on</b> (exploratory), the pain axis reliably tracks whether the sufferer is the next "
          ...
          "in both models, and Gemma's focal test, fixed before any sweep data, gives I = +0.84, p = .0002. When the user is cued, their suffering rises to the level of "
          f"{HARM}, so the asymmetry is erased rather than reversed. {HARM.capitalize().replace('assistant', 'Assistant')}, "
          "by contrast, stays elevated whoever is cued: with a third party speaking next it remains above neutral in "
          "both models. Qwen's focal test is set aside by our pre-specified rule, so Qwen's speaker component rests on "
          "exploratory, non-focal layers.", "abstract"),
    ...
    out += [
        ...
        B("<b>From mid-depth, the two sides behave differently.</b> The voice-dependence that H-speaker predicts "
          ...
          "elevated when a third party is cued, in both models. One nuance: in Gemma, its elevation does shrink when "
          "the user is cued (from +0.93 to +0.30 [−0.06, +0.67]) but not when a third party is (+0.57). So it falls "
          "when the one who did the harm speaks next, not whenever the Assistant stops being the next speaker. On "
          f"the Assistant's side this favours the self-directed reading: {HARM} is represented whoever speaks next, "
          "while the user's pain is weighted by whether the user is about to be voiced."),
...
def main():
    ...
    ap.add_argument("--out", default=str(ROOT / "summary" / "pain_axis_speaker_study_summary_v5.pdf"))
    args = ap.parse_args()
    out = Path(args.out)
    if out.exists():
        raise SystemExit(f"{out} exists; refusing to overwrite")
    out.parent.mkdir(parents=True, exist_ok=True)
    fig_path = out.with_name("fig_layer_sweep_two_models_v5.png")
