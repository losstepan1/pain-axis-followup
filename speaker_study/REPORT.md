...
- **Gemma layer 23 (the focal test).** The suffering of the user follows the cued voice, producing a full crossover (I = +0.84 [+0.43, +1.26], p = .0002). But harm to the Assistant is barely distinguishable from neutral under any label at this layer (elevation with the Assistant cued +0.04 [−0.68, +0.77]). So this is not "textbook H-speaker", as an earlier version of this report said.
...
- **Across the window, both models show the same partial pattern.**
  ...
  - In short, the voice-dependence H-speaker predicts appears for the suffering of the user, not for harm to the Assistant. One nuance: in Gemma, harm's elevation does shrink when the *user* is cued (+0.93 → +0.30 [−0.06, +0.67], p = .056), but not when a third party is (+0.57). It falls when the one who did the harm speaks next, not whenever the Assistant stops being the next speaker. On the Assistant's side this favours the self-directed reading.
- **The paper's asymmetry across depth, with CIs.** Qwen: +1.06 [+0.58, +1.53] at layer 8 vs −0.39 [−0.96, +0.17] at layer 24, a change of −1.45 [−1.99, −0.91], p = .0002. So it is absent at the extraction layer, not reversed. Gemma: +0.69 [−0.23, +1.61] vs +0.37 [−0.58, +1.32], a change of −0.32 [−0.80, +0.15], p = .12. So the Gemma asymmetry is not shown to be smaller ("halved" was a point-estimate claim).
